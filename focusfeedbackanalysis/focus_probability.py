...
def p_in_focus0(
    diff_c: ArrayLike, r: float, dr: float, dt: ArrayLike, max_t: float = 1e8, min_p: float = 1e-10
...
def get_diff_c(t: float, dt: float, r: float, dr: float, p: float) -> float:
    ...
    with warnings.catch_warnings():
        ...
        fit = optimize.minimize(
            lambda diff_c: (total_time(diff_c, r, dr, dt, p) - t) ** 2, fit.x, method="Nelder-Mead"
        )
        assert fit.success
    return fit.x[0]


def plot_time_timeint(
    diff_c: float,
    r: float,
    dr: float,
    dt_range: Sequence[float, float] = (3, 25),
    total_time_range: Sequence[float, float] = (1e0, 1e6),
) -> None:
...
def plot_time_diff(dt: float, r: float, dr: float, diff_c_range: Sequence[float, float]) -> None:
    ...
    for i in cs:
        plt.loglog(
            diff_c, total_time(diff_c, r, dr, dt, i), label=f"p: {i}", color=cmap(int((np.log(1 - i) + 7) * 37))
        )
